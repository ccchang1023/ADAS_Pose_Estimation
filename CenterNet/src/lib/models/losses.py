...
#   print("regr shape:",regr.size())         #(1,7,64,64)
#   print("gt regr shape:",gt_regr.size())   #(1,7,64,64)
...
class RegLoss_without_ind(nn.Module):
  '''
      output (batch x dim x h x w)
      mask (batch x max_objects)
      target (batch x max_objects x dim)
  '''
  def __init__(self):
    super(RegLoss_without_ind, self).__init__()
